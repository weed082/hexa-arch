...
func (s *Server) ChatService(stream pb.ChatService_ChatServiceServer) error {
	...
	for {
		...
		case CREATE_ROOM_REQ:
			s.chatPool.RegisterJob(s.createRoomJob(roomIdxs, c)) // TODO: get roomIdx after inserting room config in db
		case JOIN_ROOM_REQ:
...
func (s *Server) createRoomJob(roomIdxs *[]int, c *Client) func() {
	...
	*roomIdxs = append(*roomIdxs, roomIdx)
	return func() {
		s.chatApp.JoinRoom(s.rooms, roomIdx, c)
		msg := &Message{&pb.MsgRes{RoomIdx: int32(roomIdx)}}
		s.chatApp.BroadcastMsg(s.rooms, msg)
	}
}
...
func (s *Server) exitRoomJob(roomIdx, userIdx int) func() {
	return func() {
		err := s.chatApp.ExitRoom(s.rooms, roomIdx, userIdx)
		if err != nil {
...
func (s *Server) exitAllRooms(roomIdx *[]int, c *Client) func() {
	return func() {
		s.chatApp.ExitAllRooms(s.rooms, roomIdx, c)
	}
}
...
func (s *Server) broadcastMsgJob(msg *pb.MsgRes) func() {
	return func() {
		s.chatApp.BroadcastMsg(s.rooms, &Message{msg})
	}
}
